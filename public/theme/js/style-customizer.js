...
jQuery(document).ready(function ($) {
  ...
  $(".iq-colorbox .iq-colorselect li").on("click", function () {
    ...
    localStorage.setItem('className', JSON.stringify(colorConfig));
  ...
  var localVal = localStorage.getItem('className');
  if (localVal) {
    var ls_color = JSON.parse(localVal).color;
    var className = JSON.parse(localVal).class;

    // var $this = $(this);

    $(`.iq-colorselect li`).removeClass('iq-colormark');
    $(`.iq-colorselect .${className}`).addClass('iq-colormark');
    var str = ls_color;
    var res = str.replace("rgb(", "");
    var res1 = res.replace(")", "");
    ls_color2 = "rgba(" + res1.concat(",", 0.1) + ")";
    ls_color3 = "rgba(" + res1.concat(",", 0.8) + ")";
    document.documentElement.style.setProperty("--iq-primary", ls_color);
    document.documentElement.style.setProperty("--iq-light-primary", ls_color2);
    document.documentElement.style.setProperty("--iq-primary-hover", ls_color3);
  }
});
