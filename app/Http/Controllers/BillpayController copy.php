...
class BillpayController extends Controller
{
    ...
    public function index(Request $post, $type, $mode="online")
    {
        ...
        $data['providers'] = Provider::where('type', $type)->where('status', "1")->where('mode', $mode)->orderBy('name')->get();
    ...
    public function bbps(Request $post, $type, $mode="online")
    {
        if (\Myhelper::hasRole('admin') || !\Myhelper::can('billpayment_service')) {
            abort(403);
        }
    ...
    public function getToken($uniqueid)
       ...
       public function getToken1($uniqueid)
      {
        ...
        $key = 'UFMwMDEyNDZlZTFhYTg5ZTAxZDM1ZTBjNWZhNmVlODAyYzVjMzA3MA==';
        $signer = new HS256($key);
        $generator = new Generator ($signer);
      // dd($payload,$key,$generator,$this->api);
        return ['token' => $generator->generate($payload), 'payload' => $payload];
                ...
                switch ($provider->api->code) {
                    ...
                    case 'paysprintbill':
                        ...
                        if(!$result['error'] && $result['response'] != ''){
                            ...
                            }else if(isset($doc->response_code) && in_array($doc->response_code, [0,10])) {
                ...
                if ($debit) {
                    ...
                    switch ($provider->api->code) {
                        ...
                        default:
                            ...
                            if (env('APP_ENV') == "server") {
                                $result = \Myhelper::curl($url, "POST", json_encode($parameter), $header, "yes", "App\Models\Report", $post->txnid);
                                //dd($result,json_encode($parameter));
                            }else{
                                $result = [
                                    'error' => true,
                                    'response' => '' 
 ...
 public function paysprintoperator(){
     ...
     $url = " https://paysprint.in/service-api/api/v1/service/fastag/Fastag/operatorsList" ;
